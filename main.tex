...
software (eg: Java, ActiveX). Thus virtour can be truly portable and accessed
...
of Artificial Intelligence and Robotics, and works on creating robots that are
useful as research platforms, as well service robots to help the humans in the
building.
...
\subsubsection{Authentication} % TODO move to security
...
completed virtour will display the certificate on the website as thumbnails on
the list (illustrated in Figure \ref{scavfig}) but they can expanded to see the
full-size image by clicking on them. Finally, if the user is the leader, they
can also control the operation of the scavenger hunt by stopping and resuming
the current task. This allows a user can stop the current scavenger hunt task,
then navigate the robot elsewhere or perform any other supported operation, and
