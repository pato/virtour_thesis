...
doing, while simultaneously using the existing the autonomous navigation
...
system to ask questions, as well as request actions. Their system was built
...
the robots, thus reducing the friction for trying our system.
...
screen size (an example of scaling can be seen in Figure \ref{fig:homepage}).
...
represented by a name and associated picture\footnote{All of our robots are
named after Futurama characters}. From here a user can select a robot to
connect to by clicking on the robot's name or image. When the user clicks on
the robot, the web client will initiate a tour request to the specified robot.
If the connection is successful the client will switch into a tour session.
